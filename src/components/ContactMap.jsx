...
// Bisa diisi lebih dari 1 nomor WA, tinggal tambah/hapus item di array ini.
...
const whatsappNumbers = [
  ...
  {
    label: "STEVI",
    number: "6285656258674",
    display: "+62 856-5625-8674",
  },
];
...
export default function ContactMap() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: "",
    message: "",
    department: whatsappNumbers[0].label,
  });

  const handleSubmit = (e) => {
    e.preventDefault();
    const target =
      whatsappNumbers.find((wa) => wa.label === formData.department) ||
      whatsappNumbers[0];
    const waMessage = `Halo, saya ${formData.name}.\n\nEmail: ${formData.email}\n\nPesan (${formData.department}):\n${formData.message}`;
    window.open(
      `https://wa.me/${target.number}?text=${encodeURIComponent(waMessage)}`,
      "_blank",
    );
  };
          ...
          >
            ...
            {contactInfo.slice(0, 2).map((info) => {
              ...
              return (
                ...
                >
                  <div
                    className={`flex-shrink-0 w-10 h-10 rounded-lg ${info.color} flex items-center justify-center group-hover:scale-110 transition-transform`}
                  >
                    <Icon size={18} />
                  </div>
                  <div>
            ...
            {/* WhatsApp - mendukung lebih dari 1 nomor */}
            <div className="flex items-start gap-4 p-4 rounded-xl hover:bg-slate-50 transition-colors group">
              <div className="flex-shrink-0 w-10 h-10 rounded-lg bg-green-100 text-green-600 flex items-center justify-center group-hover:scale-110 transition-transform">
              ...
              <div className="flex-1">
                ...
                <div className="space-y-1">
                  {whatsappNumbers.map((wa) => (
                    <a
                      key={wa.label}
                      href={`https://wa.me/${wa.number}?text=${encodeURIComponent(
                        `Halo, saya ingin menanyakan informasi ${wa.label} di Grand Sya Hotel Palu.`,
                      )}`}
                      target="_blank"
                      rel="noopener noreferrer"
                    ...
                    >
                      ...
                      <span className="font-medium">{wa.display}</span>
                    </a>
                  ))}
                </div>
              </div>
            ...
            {/* Contact Cards (Email, Resepsionis) */}
            {contactInfo.slice(2).map((info) => {
              const Icon = info.icon;
              const Wrapper = info.link ? "a" : "div";
              const wrapperProps = info.link
                ? {
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <div className="relative">
                  ...
                  <select
                    ...
                    className="w-full appearance-none bg-white border border-slate-200 rounded-xl pl-10 pr-9 py-2.5 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-amber-500/50 focus:border-amber-500 transition-all"
                  >
                    {whatsappNumbers.map((wa) => (
                      <option key={wa.label} value={wa.label}>
                        {wa.label}
                      </option>
                    ))}
                  </select>
